...
## 2024-05-19 - N+1 Query / Full Table Scan in ReorderPointForecaster
**Learning:** ReorderPointForecaster was pulling every single PurchaseOrder into memory using `findAll()` and filtering them in a loop (`$po->tenantId !== $tenantId || $po->getStatus() !== PurchaseOrderStatus::Received`). In a system with many POs, this causes extreme memory exhaustion and very slow execution times due to mapping models to domain objects for every record, just to throw them away.
**Action:** Pushed the filtering down to the database level by introducing `findByTenantAndStatus` to the `PurchaseOrderRepositoryInterface` and its Eloquent implementation. The filtering now happens in SQL, eagerly loads items, and only instantiates the necessary domain objects, dropping query and hydration time significantly.
## 2024-05-19 - N+1 Query / Full Table Scan in ReorderPointForecaster
**Learning:** ReorderPointForecaster was pulling every single PurchaseOrder into memory using `findAll()` and filtering them in a loop (`$po->tenantId !== $tenantId || $po->getStatus() !== PurchaseOrderStatus::Received`). In a system with many POs, this causes extreme memory exhaustion and very slow execution times due to mapping models to domain objects for every record, just to throw them away.
**Action:** Pushed the filtering down to the database level by introducing `findByTenantAndStatus` to the `PurchaseOrderRepositoryInterface` and its Eloquent implementation. The filtering now happens in SQL, eagerly loads items, and only instantiates the necessary domain objects, dropping query and hydration time significantly.
