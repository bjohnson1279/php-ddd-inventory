...
## 2024-05-24 - SQL Injection in JSON Query
**Vulnerability:** SQL Injection in `EloquentLedgerRepository` via `whereRaw("metadata->>'locationId' = ?", [$locationId])`.
**Learning:** Using raw SQL expressions for JSON columns bypasses Eloquent's built-in parameterization and safety mechanisms, making it vulnerable to injection if the underlying PDO doesn't sanitize the JSON path string properly.
**Prevention:** Always use Laravel Eloquent's built-in JSON query syntax (e.g., `where('metadata->locationId', $locationId)`) which is safer, prevents SQL injection, and ensures cross-database compatibility.
