...
## 2024-11-20 - ZPL Injection in thermal printing endpoint
**Vulnerability:** The `/api/hardware/print-thermal` endpoint constructed ZPL (Zebra Programming Language) strings by directly interpolating user input (`$type` and `$barcode`). Because ZPL relies on control characters (`^` and `~`) to denote commands, an attacker could include these characters in the input to execute arbitrary printer commands (e.g., bypassing business logic or causing denial of service on the thermal printer).
**Learning:** Constructing strings for specialized hardware languages (like ZPL or EPL) from unsanitized user input introduces injection vulnerabilities analogous to Cross-Site Scripting (XSS) or SQL injection.
**Prevention:** Always sanitize user input prior to interpolating it into hardware command strings. For ZPL, explicitly strip out control characters like `^` and `~` using functions like `str_replace()`.
