...
function requireAuth(?string $resource = null, ?string $action = null): void
{
    ...
    if ($resource !== null && $action !== null) {
        $reqRes = strtolower($resource);
        $reqAct = strtolower($action);
        ...
        if (!$hasPermission) {
            ...
            echo json_encode(['error' => "Forbidden: Missing permission '{$required}'"]);
            exit;
        }
    }
...
if ($method === 'POST' && $uri === '/api/inventory/dispatch') {
    requireAuth('inventory', 'dispatch');
...
if ($method === 'POST' && $uri === '/api/inventory/transfer') {
    requireAuth('inventory', 'transfer');
...
if ($method === 'POST' && $uri === '/api/shipping/quote') {
    ...
    $base = (int)round($weightKg * 450);
    echo json_encode([[
        'carrier' => $carrier,
        'serviceLevel' => $input['serviceLevel'] ?? 'GROUND',
        'baseRateCents' => $base,
        'fuelSurchargeCents' => (int)round($base * 0.12),
        'totalRateCents' => (int)round($base * 1.12),
...
if ($method === 'POST' && $uri === '/api/shipping/bol') {
    ...
    echo json_encode([
        'bolNumber' => $bolNumber,
        'carrier' => $input['carrier'] ?? 'FEDEX',
        'originAddress' => $input['originAddress'] ?? 'Warehouse A, Austin TX',
        'destinationAddress' => $input['destinationAddress'] ?? 'Distribution Center, Chicago IL',
        'weightKg' => (float)($input['weightKg'] ?? 10.0),
        'totalPackages' => (int)($input['totalPackages'] ?? 1),
        'status' => 'GENERATED',
        'pdfBase64' => base64_encode("BILL OF LADING\nBOL: {$bolNumber}"),
        'createdAt' => (new DateTimeImmutable())->format(DATE_ATOM)
    ]);
    exit;
}
...
if ($method === 'POST' && $uri === '/api/erp/sync') {
    ...
    $totalAmount = array_reduce($lines, fn($acc, $l) => $acc + (int)($l['amountCents'] ?? 0), 0);
    echo json_encode([
        'success' => true,
        'provider' => $provider,
        'externalJournalId' => "EXT-{$provider}-" . rand(10000, 99999),
        'postedAmountCents' => $totalAmount,
        'lineCount' => count($lines),
        'message' => "Successfully posted " . count($lines) . " lines to {$provider}",
        'syncedAt' => (new DateTimeImmutable())->format(DATE_ATOM)
    ]);
    exit;
}
...
if ($method === 'POST' && $uri === '/api/rma/inspect') {
    ...
    echo json_encode([
        'success' => true,
        'rmaNumber' => $input['rmaNumber'] ?? 'RMA-1001',
        'sku' => $input['sku'] ?? 'SKU-1001',
        'disposition' => $disp,
        'actionTaken' => $disp === 'RESTOCK' ? 'Returned to available bin' : ($disp === 'REFURBISH' ? 'Moved to quarantine repair bin' : 'Inventory written off in ledger'),
        'notes' => $input['notes'] ?? 'Inspection completed',
...
if ($method === 'POST' && $uri === '/api/supplier/asn') {
    ...
    echo json_encode([
        'success' => true,
        'asnNumber' => $input['asnNumber'] ?? 'ASN-101',
        'supplierId' => $input['supplierId'] ?? 'SUP-101',
        'expectedDelivery' => $input['expectedDelivery'] ?? (new DateTimeImmutable())->format(DATE_ATOM),
...
if ($method === 'POST' && $uri === '/api/hardware/print-thermal') {
    ...
    echo json_encode([
        'success' => true,
        'jobId' => 'PRINT-JOB-' . rand(1000, 9999),
        'printerName' => $input['printerName'] ?? 'Zebra-ZT411',
        'zplCode' => $zpl,
...
if ($method === 'POST' && $uri === '/api/digital-twin/simulate') {
    ...
    $totalOrders = $waves * 25;
    echo json_encode([
        'scenarioId' => 'SIM-' . rand(1000, 9999),
        'durationSeconds' => 3600,
        'totalOrdersProcessed' => $totalOrders,
        'averageFulfillmentTimeMinutes' => round(12.5 / ($pickers / 5), 1),
        'bottleneckBinId' => 'BIN-B-104',
        'throughputPerHour' => round($totalOrders / 2, 1),
