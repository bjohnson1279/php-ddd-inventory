...
class RecordCountItem
{
    ...
    public function execute(string $countId, string $skuValue, string $locationIdValue, int $quantityValue): void
    {
        ...
        $locationId = new LocationId($locationIdValue);
        $quantity = new Quantity($quantityValue);

        // Record the counted item in the aggregate
        $inventoryCount->recordCount($sku, $locationId, $quantity);
