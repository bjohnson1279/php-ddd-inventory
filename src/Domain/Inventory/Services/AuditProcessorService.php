...
class AuditProcessorService
{
    ...
    public function runAudit(string $tenantId): array
    {
        ...
        if ($storeDomain && $accessToken) {
            ...
            // Build reference IDs to filter existing discrepancies
            $possibleReferenceIds = [];
            foreach ($skus as $sku) {
                foreach ($locMappings as $locMap) {
                    $possibleReferenceIds[] = "{$sku}:{$locMap->our_location_id}";
                }
            }

            // Pre-fetch all open Shopify discrepancies for this tenant
