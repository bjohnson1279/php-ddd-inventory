...
class TenantConnectionPool
{
    ...
    public function getConnection(string $tenantId): \Illuminate\Database\Connection
    {
        ...
        if (!$entry) {
            ...
            $dbPassword = getenv('DB_PASSWORD') !== false && getenv('DB_PASSWORD') !== '' ? getenv('DB_PASSWORD') : (getenv('APP_ENV') === 'testing' ? 'postgres' : throw new \RuntimeException('DB_PASSWORD is required'));
    ...
    public function evict(string $tenantId): void
    {
        if (isset($this->cache[$tenantId])) {
            ...
            try {
                ...
                error_log('[TenantConnectionPool.php] Failed to disconnect tenant connection: ' . $e->getMessage());
            }
